...
public class DocumentController {
    ...
    private Map<String, Object> buildPreviewResponse(FileUpload file, Integer pageNumber, boolean preferSinglePagePreview) {
        ...
        // 为非文本类型追加文件元信息作为 content，让前端调试日志更有意义
        long fileSize = file.getTotalSize();
        String sizeStr = fileSize < 1024 ? fileSize + "B" :
                fileSize < 1048576 ? String.format("%.1fKB", fileSize / 1024.0) :
                String.format("%.1fMB", fileSize / 1048576.0);
        String metaContent = String.format("文件类型: %s | 大小: %s | MD5: %s",
                extension.toUpperCase(), sizeStr, file.getFileMd5());
        payload.put("content", metaContent);
    ...
    /**
     * 流式代理 MinIO 文件，避免前端直接访问 MinIO 导致的跨域问题
     * 
     * @param fileMd5 文件MD5
     * @param userId 当前用户ID
     * @param orgTags 组织标签
     * @return 文件流响应
     */
    @GetMapping("/stream/{fileMd5}")
    public ResponseEntity<?> streamFile(
            @PathVariable String fileMd5,
            @RequestAttribute("userId") String userId,
            @RequestAttribute("orgTags") String orgTags) {
        
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("STREAM_FILE");
        try {
            LogUtils.logBusiness("STREAM_FILE", userId, "接收到文件流式代理请求: fileMd5=%s", fileMd5);
            ...
            if (file == null) {
                monitor.end("流式代理失败：文件不存在或无权限访问");
                Map<String, Object> response = new HashMap<>();
                response.put("code", HttpStatus.NOT_FOUND.value());
                response.put("message", "文件不存在或无权限访问");
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
            ...
            LogUtils.logFileOperation(userId, "STREAM", file.getFileName(), fileMd5, "SUCCESS");
            monitor.end("文件流式代理成功");
            ...
            LogUtils.logBusinessError("STREAM_FILE", userId, "文件流式代理失败: fileMd5=%s", e, fileMd5);
            monitor.end("流式代理失败: " + e.getMessage());
            Map<String, Object> response = new HashMap<>();
            response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
            response.put("message", "文件流式代理失败: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
        }
    }

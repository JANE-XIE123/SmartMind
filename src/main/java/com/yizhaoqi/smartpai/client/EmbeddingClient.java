...
public class EmbeddingClient {
    ...
    private String callApiOnce(List<String> batch) {
        ...
        return buildClient(provider).post()
                ...
                .retryWhen(Retry.backoff(3, Duration.ofSeconds(1))
                        ...
                        .doBeforeRetry(signal -> logger.warn("重试 Embedding API 调用 - 尝试: {}/{}, 错误: {}",
                                signal.totalRetries() + 1, signal.totalRetriesInARow() + 1,
                                signal.failure().getMessage())))
                .onErrorMap(WebClientResponseException.class, this::handleEmbeddingApiError)
    ...
    /**
     * 处理 Embedding API 的错误，区分不同类型的错误
     */
    private Exception handleEmbeddingApiError(WebClientResponseException ex) {
        var status = ex.getStatusCode();
        
        if (status.value() == 401) {
            logger.error("Embedding API 认证失败 (401 Unauthorized) - 请检查 API Key 是否有效");
            return new RuntimeException("Embedding API 认证失败：请检查 API Key 配置是否正确", ex);
        } else if (status.value() == 429) {
            logger.warn("Embedding API 请求过于频繁 (429 Too Many Requests)");
            return new RuntimeException("Embedding API 请求频率超限，请稍后重试", ex);
        } else if (status.value() >= 500) {
            logger.warn("Embedding API 服务端错误 ({}): {}", status.value(), ex.getResponseBodyAsString());
            return new RuntimeException("Embedding API 服务端错误: HTTP " + status.value(), ex);
        } else {
            logger.error("Embedding API 客户端错误 ({}): {}", status.value(), ex.getResponseBodyAsString());
            return new RuntimeException("Embedding API 错误: HTTP " + status.value() + " - " + ex.getResponseBodyAsString(), ex);
        }
    }

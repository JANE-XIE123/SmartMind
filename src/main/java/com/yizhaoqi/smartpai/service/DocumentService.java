...
public class DocumentService {
    ...
    /**
     * 生成文件访问链接（MinIO 预签名 URL，前端可直接访问，无需后端代理）
     *
     * @param fileMd5 文件MD5
     * @return MinIO 预签名 URL（公开可访问）
     */
    public String generateDownloadUrl(String fileMd5) {
        ...
        try {
            ...
            // 使用公网 MinioClient 生成预签名 URL（签名与公网地址匹配，浏览器可直接访问）
            ...
            logger.info("生成预签名URL: fileMd5={}, fileName={}, url={}",
                    fileMd5, fileUpload.getFileName(), presignedUrl);
            return presignedUrl;
        } catch (Exception e) {
